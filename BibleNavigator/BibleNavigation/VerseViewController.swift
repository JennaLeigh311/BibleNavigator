...
class VerseViewController: BaseCollectionViewController {
    ...
    override var data: [String] {
        if verses == 0 {
            print(
                "No verses for chapter \(String(chapter)) of book \(book?.title ?? "Unknown")"
            )
            return []
        }
    ...
    override func didSelect(item: String) {
        ...
        let oliveTreeUrl =
            "olivetree://bible/\(book?.id ?? 0).\(chapter).\(item)"
        if let url = URL(string: "\(oliveTreeUrl)"),
            UIApplication.shared.canOpenURL(url)
        {
            ...
            let appStoreURL = URL(
                string: "https://www.olivetree.com/blog/apps/ios/"
            )!
